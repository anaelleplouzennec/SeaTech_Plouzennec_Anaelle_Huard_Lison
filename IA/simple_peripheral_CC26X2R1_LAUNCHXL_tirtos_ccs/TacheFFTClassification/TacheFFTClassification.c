...
void TacheFFTClassification_CreateTask(void){
    ...
    Task_construct(&TacheFFTClassification, TacheFFTClassification_taskFxn,&taskParams, NULL);
...
static void TacheFFTClassification_taskFxn(UArg a0, UArg a1)
{
    ...
    for (;;)
    {
        ...
        //On utilise FFTSerieIm comme serie des parties imaginaires du signal temporel que l’on met a 0
        for (int i=0; i<256; i++)
        {
            FFTSerieIm[i] = 0;
        }
